<%@ Page Language="C#" Inherits="System.Web.Mvc.ViewPage<dynamic>" %> <%@ Import Namespace="JiuZhou.ControllerBase" %><%@ Import Namespace="JiuZhou.Model" %><%@ Import Namespace="JiuZhou.Common" %><%@ Import Namespace="JiuZhou.MySql" %><%@ Import Namespace="JiuZhou.HttpTools" %><%@ Import Namespace="JiuZhou.Cache" %>
<%
...
div.webuploader-pick{height:40px; }
#filePicker2 .webuploader-pick{height:24px; }
#uploader p.title{z-index:3}
</style>
</head>
<body>
...
<tbody>
  <tr>
    ...
    <%}
      ...
      {
    %>
       &nbsp;<input type="radio" name="checktype" onclick="checkedthis(this)" value="0"<%if(Info.check_state==0){Response.Write(" checked=\"checked\"");}%> /> 审核通过
       &nbsp;|&nbsp;
       <input type="radio" name="checktype" onclick="checkedthis(this)" value="1"<%if(Info.check_state==1){Response.Write(" checked=\"checked\"");}%>/> 审核不通过
    <%} %>
    </p>
    </td>
    <td>
    <input type="checkbox" name="p_is_prescription_drug" value="1" <%=Info.is_drug>0?" checked=\"checked\"":""%>/> 处方药
    &nbsp;|&nbsp;
    <input type="checkbox" name="isOnSale" value="1" <%=Info.is_on_sale>0?" checked=\"checked\"":""%>/> 商品上架
    &nbsp;|&nbsp;
    <input type="checkbox" name="isVisible" value="0" <%=Info.is_visible<1?" checked=\"checked\"":""%>/> 放入回收站
    </td>
  </tr>
  <tr>
    <td style="width:3%">&nbsp;</td>
  ...
  <tr>
    ...
    <td class="inputText"><input type="text" name="KeyForSearch" value="<%=Info.search_key%>" class="input" style="width:520px"/></td>
    <td>多个关键词用逗号隔开</td>
  </tr>
  <tr>
    <td style="width:3%">&nbsp;</td>
    <td class="lable">商品卖点</td>
    <td class="inputText"><input type="text" name="promotion" value="<%=Info.sales_promotion%>" class="input" style="width:520px"/></td>
...
<tbody>
  <tr>
    ...
    <td colspan="2"><input type="text" name="p_spec" value="<%=Info.product_spec%>" class="input" />
  ...
  <tr>
    ...
    <td colspan="2"><input type="text" name="p_common_name" value="<%=Info.common_name%>" class="input" />
  ...
  <tr>
	...
	<input type="text" id="box-ihours" name="ihours" value="<%=DateTime.Parse(Info.invalid_date).ToString("HH")%>" class="input" style="width:40px" title="数字0至23"/> 时
    <input type="text" id="box-iminutes" name="iminutes" value="<%=DateTime.Parse(Info.invalid_date).ToString("mm")%>" class="input" style="width:40px" title="数字0至59"/> 分
<script type="text/javascript">
Atai.addEvent(window,"load",function(){
  ...
  <tr>
    ...
    <td colspan="2"><input type="text" name="monthClicks" value="<%=Info.month_click_count%>" class="input" style="width:80px"/>
    </td>
  </tr>
</tbody>
</table>
...
<tbody>
  <tr>
    ...
    <td colspan="2"><input type="text" id="attr-code" name="code" value="<%=Info.product_code%>" onblur="checkCode(this)" class="input" style="width:164px"/>
    ...
    &nbsp;重量 <b style="color:#ff6600">*</b>&nbsp;<input type="text" id="attr-weight" name="weight" value="<%=Info.product_weight.ToString("0")%>" class="input" style="width:80px" onkeyup="chkNumber(this,'重量', 'tips-sale')"/> 克
    &nbsp;
  ...
  <tr>
    ...
    <td class="2"><input type="text" id="attr-member" name="member_price" value="<%=Info.sale_price%>" class="input" onkeyup="chkNumber(this,'售价', 'tips-sale')" onchange="changeMemberPrice(this)" style="width:80px"/> 元
    &nbsp;&nbsp;
    手机 <b style="color:#ff6600">*</b>&nbsp;<input type="text" id="attr-mobile" name="mobile_price" value="<%=Info.mobile_price%>" class="input" onkeyup="chkNumber(this,'手机专享价', 'tips-sale')" style="width:80px"/> 元
    &nbsp;&nbsp;
    限购数量  <input type="text" id="attr-purchase" name="max_buy_num" value="<%=Info.max_buy_num%>" class="input" onkeyup="chkNumber(this,'限购数量', 'tips-sale')" style="width:80px"/> 小于或等于0表示不限购
    </td>
  </tr>
  ...
  <tr>
    ...
    <td colspan="2">
    <input type="text" id="attr-promotion" name="promotion_price" value="<%=Info.promotion_price%>" class="input" onkeyup="chkNumber(this,'促销价', 'tips-sale')" style="width:80px"/> 元
    &nbsp;&nbsp;
    从
	...
	<input type="text" id="box-shours" name="shours" value="<%=DateTime.Parse(Info.promotion_bdate).ToString("HH")%>" class="input" style="width:40px" title="数字0至23"/> 时
    <input type="text" id="box-sminutes" name="sminutes" value="<%=DateTime.Parse(Info.promotion_bdate).ToString("mm")%>" class="input" style="width:40px" title="数字0至59"/> 分
...
至
	...
	<input type="text" id="box-ehours" name="ehours" value="<%=DateTime.Parse(Info.promotion_edate).ToString("HH")%>" class="input" style="width:40px" title="数字0至23"/> 时
    <input type="text" id="box-eminutes" name="eminutes" value="<%=DateTime.Parse(Info.promotion_edate).ToString("mm")%>" class="input" style="width:40px" title="数字0至59"/> 分
<script type="text/javascript">
...
if (Info.product_id > 0)
{
    ...
    =
    <input type="text" id="integral_view" value="<%=Info.give_integral%>" class="input" style="width:50px" disabled="disabled"/> 积分
<script type="text/javascript">
...
<thead>
  <tr>
    ...
    <th colspan="3" style="width:250px;text-align:right;font-weight:100">虚拟库存(正数为增加,负数为减少) <input type="text" v="stock" value="" class="input" style="width:40px"/></th>
    <th style="width:50px;font-weight:100">&nbsp;</th>
    <th style="width:50px;font-weight:100"><input type="text" v="weight" value="" class="input" style="width:20px;"/> 克</th>
    <th style="width:75px;font-weight:100"><input type="text" v="member" value="" class="input" style="width:45px"/> 元</th>
    <th style="width:75px;font-weight:100"><input type="text" v="mobile" value="" class="input" style="width:45px"/> 元</th>
    <th style="width:75px;font-weight:100"><input type="text" v="promotion" value="" class="input" style="width:45px"/> 元</th>
    <th style="font-weight:120"><div class="console" style="margin:0;border:0;"><a href="javascript:;" onclick="setSkuAttr(this)">批量设置</a></div></th>
  </tr>
</thead>
<tbody>
  <tr>
...
%>
  <tr>
    ...
    <td style="width:80px"><input type="text" v="skuName" value="<%=sku.sku_name%>" class="input" style="width:80px"/></td>
    <td style="width:160px"><input type="text" v="skuProductNumber" value="<%=sku.sku_code%>" onblur="checkCode(this)" class="input" style="width:160px"/></td>
    <td style="width:50px"><input type="text" v="skuStock" value="<%=sku.virtual_sku_stock%>" class="input" style="width:40px"/></td>
    <td style="width:50px"><input type="text" v="skuvStock" value="<%=sku.sku_stock%>" class="input" style="width:40px" disabled/></td>
    <td style="width:50px"><input type="text" v="skuWeight" value="<%=sku.sku_weight%>" class="input" style="width:40px"/></td>
    <td style="width:75px"><input type="text" v="skuMemberPrice" value="<%=sku.sale_price%>" class="input" style="width:40px"/> 元</td>
    <td style="width:75px"><input type="text" v="skuMobilePrice" value="<%=sku.mobile_price%>" class="input" style="width:40px"/> 元</td>
    <td style="width:75px"><input type="text" v="skuPromotionPrice" value="<%=sku.promotion_price%>" class="input" style="width:40px"/> 元</td>
...
<tbody>
  <tr>
    <td style="width:66px;">+ <input type="text" id="addRowCount" onclick="this.select()" value="1" class="input" style="width:20px;text-align:center;"/> 行</td>
...
<tbody>
  ...
  <tr>
	...
	<input type="text" id="fare-shours" name="fshours" value="<%=DateTime.Parse(Info.free_fare_stime).ToString("HH")%>" class="input" style="width:40px" title="数字0至23"/> 时
    <input type="text" id="fare-sminutes" name="fsminutes" value="<%=DateTime.Parse(Info.free_fare_stime).ToString("mm")%>" class="input" style="width:40px" title="数字0至59"/> 分
...
至
	...
	<input type="text" id="fare-ehours" name="fehours" value="<%=DateTime.Parse(Info.free_fare_etime).ToString("HH")%>" class="input" style="width:40px" title="数字0至23"/> 时
    <input type="text" id="fare-eminutes" name="feminutes" value="<%=DateTime.Parse(Info.free_fare_etime).ToString("mm")%>" class="input" style="width:40px" title="数字0至59"/> 分
<script type="text/javascript">
$(function(){
	var tips=$("#tips-fare");
...
{
%>
		<dt><input type="hidden" id="upload-image<%=i%>" v="image" imgid="0" isdef="0" value="" />
			<div id="upload-box-image<%=i%>" onclick="ajaxUploadClick(this, Atai.$('#upload-image<%=i%>'))"><br/><br/><br/>图片尺寸 800×800 像素<br/>点击此处上传图片</div>
			<p><a href="javascript:;" onclick="showImageListBox(setProImage<%=i%>)">去图库选图</a>
            &nbsp;&nbsp;<a href="javascript:;" onclick="$('#upload-box-image<%=i%>').click()">本地上传</a>
            </p>
			<p class="op"><a href="javascript:;" onclick="deleteImage('upload-image<%=i%>','<%=i%>')">删除图片&nbsp;</a>
...
<thead>
  <tr>
    <th style="width:3%">&nbsp;</th>
    <th class="lable"><input type="submit" id="loadding3" value="  确定提交  " class="submit"/></th>
