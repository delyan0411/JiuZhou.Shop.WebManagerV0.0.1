...
<div class="position">
当前位置：
<a href="/" title="管理首页">管理首页</a> &gt;&gt; <span>页面设置(手机)</span>
</div>
...
<%
    ...
    var ress = GetRecommendPositionByCode.Do("0", 1);
...
%>
  <tr>
    <td><%=item.rp_id%></td>
    <td><a id="rpname" href="/mpolymeric/index?pid=<%=item.rp_id%>&useplat=1"><%=item.rp_name.Trim()%>(<%=item.rp_id %>)</a></td>
    ...
    <td>
    <a href="javascript:;" onclick="showRecommendPageEditorBox(event,'<%=item.rp_id%>',$('#pos-<%=item.rp_id%>').html(),1,1)">修改名称</a>
    <%if (_isallow)
      { %>
    &nbsp;
    <a id="poschg" href="/mpolymeric/index?pid=<%=item.rp_id%>&useplat=1">查看/设置页面</a>
       <%} %>
    &nbsp;
    <a id="itemchg" href="/mpolymeric/list?posid=<%=item.rp_id%>&useplat=1">查看/设置主题</a>
    &nbsp;
...
<%if (_isallow)
      { %>
	<div class="console">
		<a href="javascript:;" onclick="showRecommendPageEditorBox(event,0,'',0,1   )"><b class="add">&nbsp;</b>新增页面</a>
	</div>
       <%} %>
</div>
</div>
