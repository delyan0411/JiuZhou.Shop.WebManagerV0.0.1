...
<script language="c#" runat="server">
	...
	public void PrintList(int parentId){
        ...
        foreach (HelpTypeInfo item in items)
        {
			...
			Response.Write("<a href=\"javascript:;\" v=\"bt\" style=\"color:"+(item.type_state==0?"#000":"#999")+"\">" + item.type_name + "</a>");
            List<HelpTypeInfo> __ems = tList.FindAll(
                delegate(HelpTypeInfo e)
                {
					return e.parent_id==item.help_type_id;
			});
			if(item.type_state==0 && (tree>2 || __ems.Count==0)){
				...
				for(int x=0;x<arr.Length;x++){
                    ...
                    foreach (HelpTypeInfo ie in tList)
                    {
				       if (ie.help_type_id ==_v){
                ...
                List<HelpTypeInfo> _emss = tList.FindAll(
                    delegate(HelpTypeInfo emxx)
                    {
						return emxx.parent_id==item.help_type_id;
				});
				if(_emss.Count<1){
					Response.Write("<a href=\"javascript:;\" onclick=\"selectThisCateg(event," + item.help_type_id + ", '"+ Utils.ToUnicode(clsName) +"')\" style=\"color:"+(item.type_state==0?"#060":"#999")+"\" title=\"选择分类\">选择分类</a>");
				}
				Response.Write("</p>");
			}
			Response.Write("</div>");
