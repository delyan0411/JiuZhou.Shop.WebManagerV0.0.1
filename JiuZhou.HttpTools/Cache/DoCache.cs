...
namespace JiuZhou.Cache
{
    ...
    public class DoCache
    {
        ...
        #region 属性
        private long _expires = 1200;//默认缓存20分钟;
        ...
        private void SetAtaiCacheKeys(SortedList<string, string> val)
        {
            ...
            HttpContext.Current.Cache.Insert(this._applicationKey, val, null, DateTime.Now.AddDays(7), TimeSpan.Zero);
        }
        #endregion
        ...
        public void SetCache(string key, object val, long expires)
        {
            if (string.IsNullOrEmpty(key))
                return;
            string _keyTem = key;
            key = JiuZhou.Common.Utils.MD5(key);
        ...
        public object GetCache(string key, bool isDecryptKey)
        {
            if (string.IsNullOrEmpty(key))
                return new object();
            if (!isDecryptKey) key = JiuZhou.Common.Utils.MD5(key);
