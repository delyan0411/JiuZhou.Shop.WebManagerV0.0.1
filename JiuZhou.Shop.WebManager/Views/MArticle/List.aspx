...
<form action="/marticle/list" method="get" onsubmit="checksearch(this)">
<input type="hidden" name="size" value="<%=DoRequest.GetQueryInt("size", 100)%>"/>
		...
		<p><select id="stype" name="classid" style="width:80px">
			<option value="0" init="true">所有分类</option>
<%
    List<ArticleTypeInfo> _items = tList.FindAll(
            delegate(ArticleTypeInfo em)
            {
                return em.parent_id == 0;
            });
    foreach (ArticleTypeInfo em in _items)
    {
		Response.Write("<option value=\""+ em.article_type_id +"\">"+ em.article_type_name +"</option>");
	}
%>
		</select>
</p><p>
...
<tbody>
<%
    List<ShortArticleInfo> infoList = (List<ShortArticleInfo>)ViewData["infoList"];
    foreach (ShortArticleInfo item in infoList)
    {
        DateTime addTime = DateTime.Parse(item.add_time);
        string classPath = "";
        ArticleTypeInfo hCateg = new ArticleTypeInfo();
        foreach (ArticleTypeInfo c in tList)
        {
			if(c.article_type_id == item.article_type_id){
...
%>
  <tr>
    <td><input type="checkbox" onclick="selectOne(this)" name="visitid" value="<%=item.article_id%>" /></td>
    <td><a href="/marticle/editor?id=<%=item.article_id%>" target="_blank"><%=item.article_title%></a>
<p style="color:#999"><%
		int _xCount=0;
		for(int x=0;x<_tem.Length;x++){
            ...
            {
            	if (em.article_type_id ==_v){
					if(_xCount>0) Response.Write(" &gt;&gt; ");
					Response.Write(em.article_type_name);
					_xCount++;
            	}
        	}
		}
%></p>
    ...
    <td><input type="text" class="input" style="width:30px;text-align:center;" onblur="resetSort('<%=item.article_id%>', this.value)" onclick="this.select()" value="<%=item.sort_no%>"/></td>
    <td><a href="javascript:;" onclick="resetstate(<%=item.article_id %>,<%=item.article_state==0?1:0 %>)"><%=item.article_state==0?"已显示":"<span style='color:red'>已隐藏</span>"%></a></td>
    <td><%=item.author_name%></td>
    <td>
    <a href="/marticle/editor?id=<%=item.article_id%>" target="_blank">编辑</a>
    &nbsp;
    <a href="javascript:;" onclick="return deleteList(<%=item.article_id%>)">删除</a>
    </td>
  </tr>
<%
	}
...
<%=ViewData["pageIndexLink2"]%>
			<div class="console">
                ...
                <a href="/marticle/editor?id=0">新增</a>
			</div>
<br/><br/><br/><br/>
</div>
...
<script type="text/javascript">
    function resetstate(id, state) {
        ...
        $.ajax({
            url: "/MArticle/ResetArticleState"
		, data: postData
        , type: "post"
		, dataType: "json"
...
function deleteList(form){
	...
	jsbox.confirm('您确定要删除这些数据吗？',function(){
		$.ajax({
			url: "/MArticle/RemoveArticleList"
			, data: postData
            , type: "post"
			, dataType: "json"
			, success: function(json){
