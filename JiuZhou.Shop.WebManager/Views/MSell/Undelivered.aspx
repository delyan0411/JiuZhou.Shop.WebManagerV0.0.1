...
$(function(){
                ...
                <thead>
                    ...
                    if (order.expired_minute <= 240)
                        Response.Write("&nbsp;<img src=\"/images/icon/clock2.png\" align=\"absmiddle\" alt\"限时秒杀\"/>");
                                ...
                                元 优惠
                                <%=order.pay_total_money - order.pay_order_money %>
                                元)</span> <span>
                                    <%=(order.pay_type == 0 && order.pay_state > 0) ? "医卡通" : order.pay_type_name%></span>
                            &nbsp; <a href="/msell/undelivered?status=2&stype=4&q=<%=order.user_id%>" target="_blank">
                                查看用户订单</a>
                            <%
                                ...
                                <dd>
                                    ...
                                    <div class="text">
                                        ...
                                        <a href="<%=config.UrlHome%><%=item.product_id%>.html" target="_blank">
                                            <%=item.product_name%></a><span style="color: Blue"><%=_itemstate%></span><br />
    ...
    <script type="text/javascript">
        function resetOrderStatus(event, orderNumber) {
            var postData = "orderNumber=" + orderNumber;
            jsbox.confirm('您确定要将订单 <span style="color:#ff6600">' + orderNumber + '</span> 退订吗？', function () {
                $.ajax({
        ...
        function resetProductID(obj, id) {
            var postData = "itemid=" + id;
            jsbox.confirm('您确定要调整该商品的ID吗？', function () {
                $.ajax({
                    url: "/MSell/ResetProductID"
			, data: postData
            , type: "post"
			, dataType: "json"
			, success: function (json) {
