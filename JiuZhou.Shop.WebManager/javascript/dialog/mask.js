...
var Popups = function (stringContainerId) {
	...
	this.initializeElement = function (stringTitleIconClassName, stringIconClassName, booleanButtons, functionCallback) {
		...
		container.show = function () {
			...
			this.style.top = ((getWindowSize()[1] - 192) / 2 + Atai.scroll().y) + "px";
			mask.show();
	...
	this.createPopup = function (stringType, stringNote, functionCallback) {
		switch (stringType) {
			case "AlertError":
				...
				var popup = this.initializeElement("titleAlert", "iconWarning", false, functionCallback);
				popup.element.innerHTML = '<div class="dialog_title">' + stringNote + '</div>';
				return popup;
				break;
			case "AlertSuccess":
				//var popup = this.initializeElement("titleAlert", "iconSuccess", false, new Function());
				var popup = this.initializeElement("titleAlert", "iconSuccess", false, functionCallback);
				popup.element.innerHTML = '<div class="dialog_title">' + stringNote + '</div>';
				return popup;
				break;
			case "Confirm":
				...
				line.innerHTML = '<div class="dialog_title">' + stringNote + '<br /><div>';
			...
			case "Prompt":
				...
				line.innerHTML = '<div class="dialog_title"><label>' + convertHTML(combination) + '</label>:<div>';
...
var AtaiBox = new Object();
AtaiBox.returnValue=false;
AtaiBox.createElement=function(tagName){
	return document.createElement(tagName);
};
AtaiBox.createPopup=function(){
	mask = new Mask();
...
AtaiBox.error=function(msg, url){
	this.createPopup();
	var callback=function(){return true};
	if(url){
		callback=function (){
			 if(url.toLowerCase()=="history") history.go(-1);
			 else window.location.href=url;
		};
	}
	popups.createPopup("AlertError", msg, callback).show();
};
AtaiBox.alert=function(msg, url){
	this.createPopup();
	var callback=function(){return true};
	if(url){
		callback=function (){
	...
	popups.createPopup("AlertSuccess", msg, callback).show();
};
AtaiBox.confirm=function(msg, callback){
	this.createPopup();
	if(!callback) callback=function(){return true;};
	popups.createPopup("Confirm",msg, callback).show();
};
AtaiBox.callbackFunction=function(val){
	alert(val);
};
AtaiBox.prompt=function(msg,callback){
	this.createPopup();
	popups.createPopup("Prompt",msg,callback).show();
};