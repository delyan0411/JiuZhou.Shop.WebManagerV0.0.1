...
<tbody>
  <tr>
    ...
    <td class="inputText"><input type="text" name="name" value="<%=Info.ass_subject%>" class="input" style="width:420px;"/></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td style="width:3%">&nbsp;</td>
    ...
    <td colspan="2">
    <input type="radio" name="AssType" onclick="checkedthis(this)" value="1"<%if(Info.ass_type>0){Response.Write(" checked=\"checked\"");}%> /> 固定搭配
    &nbsp;|&nbsp;
    <input type="radio" name="AssType" onclick="checkedthis(this)" value="0"<%if(Info.ass_type==0){Response.Write(" checked=\"checked\"");}%>/> 自由搭配
    </td>
  </tr>
  <tr>
    <td style="width:3%">&nbsp;</td>
...
%>
  <tr>
    <td>&nbsp;</td>
    <td style="width:70px;height:74px;"><input type="hidden" v="main-product-id" value="<%=product.product_id %>"/>
<a href="<%=config.UrlHome%><%=product.product_id %>.html" target="_blank" title="<%=product.product_name%>"><img src="<%=FormatImagesUrl.GetProductImageUrl(product.img_src, 120, 120) %>" style="width:60px;height:60px" alt=""/>
</a>
    </td>
    <td>
...
%>
  <tr>
    <td>&nbsp;</td>
    <td style="width:70px;height:74px;"><input type="hidden" v="item-product-id" value="<%=product.product_id %>"/>
      <a href="<%=config.UrlHome%><%=product.product_id %>.html" target="_blank" title="<%=product.product_name%>"><img src="<%=FormatImagesUrl.GetProductImageUrl(product.img_src, 120, 120) %>" style="width:60px;height:60px" alt=""/>
      </a>
    </td>
    <td>
    ...
    <p style="font-family:Arial;margin-top:6px;color:#666"><del>￥<%=product.sale_price%></del></p></td>
    <td>
	  <p class="move-links">
