...
<script language="c#" runat="server">
	...
	public void PrintList(int parentId){
        ...
        foreach (TypeList item in items)
        {
			...
			if(item.is_visible>0){
				...
				Response.Write("<a href=\"javascript:;\" onclick=\"selectThisCateg(event," + item.product_type_id + ", '"+ Utils.ToUnicode(clsName) +"', '"+ item.product_type_path +"')\" style=\"color:"+(item.is_visible>0?"#060":"#999")+"\" title=\"选择分类\">选择分类</a>");
				Response.Write("</p>");
			}
			Response.Write("</div>");
    ...
    <script type="text/javascript">
        ...
        var _showPath = false;
        function selectCategBox(event, inputId, showInputId, path) {
            _returnInputId = inputId; _showInputId = showInputId; _showPath = path;
        ...
        function selectThisCateg(event, typeId, name, path) {
