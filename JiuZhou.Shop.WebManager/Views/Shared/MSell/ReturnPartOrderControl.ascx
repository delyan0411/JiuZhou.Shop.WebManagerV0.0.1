...
%>
<div id="returnPartOrderState-boxControl" class="moveBox" style="height:700px;width:700px;">
	<div class="name">
		部分退货
		<div class="close" v="atai-shade-close" title="关闭">&nbsp;</div>
	</div>
	<div class="clear">&nbsp;</div>
<form action="" onsubmit="return PostReturnPartOrderStatus(this)">
<table width="100%" border="0" cellspacing="2" cellpadding="0">
  ...
  <tr>
    ...
    <td>
    <input type="text" id="return-orderNumber" class="input" value="" name="orderNumber2"/>
...
<script type="text/javascript">
    function PostReturnPartOrderStatus(form) {
        var _data = getPostDB(form);
         jsbox.confirm('您确定要将订单 <span style="color:#ff6600">' + $(form).find("input[name='orderNumber']").val() + '</span> 退货吗？', function () {
            $.ajax({
                url: "/MSell/ReturnPartOrderStatus"
			, data: _data
            , type: "post"
			, dataType: "json"
			, success: function (json) {
			    if (json.error) {
			        $(_returnPartOrderStatusBoxDialog.dialog).find("span[class='tips-text']").html(json.message);
			    } else {
...
var _returnPartOrderStatusBoxDialog = false;
function returnPartOrderStatusBox(orderNumber, state, remark) {

    var boxId = "#returnPartOrderState-boxControl";
    var box = Atai.$(boxId);
    ...
    _returnPartOrderStatusBoxDialog = _dialog;

    $(_returnPartOrderStatusBoxDialog.dialog).find("input[name='orderNumber']").val(orderNumber);
    $(_returnPartOrderStatusBoxDialog.dialog).find("input[name='orderNumber2']").val(orderNumber);

    $(_returnPartOrderStatusBoxDialog.dialog).find("input[name='serviceState']").each(function () {
        ...
        url: "/MSell/GetOrderProducts"
			...
			, success: function (json) {
			    if (json.error) {
			        ...
			        var o = $(_returnPartOrderStatusBoxDialog.dialog).find("div[v='data-list-options'] ul");
			        o.append(html);
			    }
			}
    });

    $(_returnPartOrderStatusBoxDialog.dialog).find("textarea[name='remark']").val(remark);
