...
<%
    ...
    List<PayTypeInfo> ptList = new List<PayTypeInfo>();
    var restype = GetPayType.Do();
    if (restype != null && restype.Body != null && restype.Body.pay_type_list != null)
        ptList = restype.Body.pay_type_list;
...
<div class="position">
当前位置：
<a href="/" title="管理首页">管理首页</a> &gt;&gt; <span>订单列表</span>
&nbsp;&nbsp;&nbsp;
</div>
<form id="sForm" action="" method="get" onsubmit="checksearch(this)">
...
<p><a href="javascript:;" onclick="changeOrderBy('TOTALMoney','<%=otype=="asc"?"desc":"asc"%>')">按价格排序</a></p>
			</div>
</form>
...
/*		List<OrderInfo> _orderList = children.FindAll(
		...
		PayTypeInfo payType = new PayTypeInfo();
		foreach(PayTypeInfo p in ptList){
			if(p.pay_type_id==order.pay_type){ payType=p;break; }
		}
%>
...
<thead>
  <tr>
    ...
    <b v="price">￥<%=order.pay_order_money + order.pay_trans_money%></b> <span v="farePrice">(含运费 <%=order.pay_trans_money%> 元  优惠 <%=order.pay_total_money - order.pay_order_money %> 元)</span>
    <span><%=(order.pay_type == 0 && order.pay_state > 0) ? "医卡通" : order.pay_type_name%></span>
    &nbsp;
    <a href="/msell/nonpay?status=1&stype=4&q=<%=order.user_id%>" target="_blank">查看用户订单</a>
   
    <%
        if ((order.pay_order_state == 1)
		&& order.pay_state==0
	){
...
<%
	...
	foreach(OrderInfo od in oList){
		...
		//vCardLogList
        /*
		List<UsersVirtualCardLogInfo> _vCardLog = vCardLogList.FindAll(
			delegate(UsersVirtualCardLogInfo em){
				return em.OrderNumber==od.order_number && em.LogText.IndexOf("退款")>=0;
		});//退款日志
		decimal backMoney=0;//退款
		foreach(UsersVirtualCardLogInfo em in _vCardLog){
			backMoney += em.Money;
		}
         * */
%>
  ...
  <tr <%=className%>>
	...
	<div class="order-hd">
        ...
        订单编号：<%=od.order_no%>
        &nbsp;(<%=shop.shop_addr%> / <%=shop.shop_name%>)
        </div>
        <div class="order-items">
        <a href="javascript:;" onclick="showItems(this)"><span>展开</span><b>收缩</b></a>
        </div>
		<div class="clear"></div>
		<div><%=od.province_name%>,<%=od.city_name%>,<%=od.county_name%>,<%=od.receive_addr%>,<%=od.receive_name%>,<%
		...
		%></div>
        <div id="remark-<%=od.order_id%>" style="background-color:#ccc"><%=od.inner_remark%></div>
    </div>
	<dl>
<%    if (items == null)
...
%>
	<dd>
		...
		<div class="text">
            ...
            编码：<%=item.product_code%>  <a href="javascript:;" onclick="resetCode(<%=od.order_id %>,<%=item.product_id %>,'<%=item.product_code %>','<%=item.product_name %>')">修改</a>            
		</div>
		<div class="unit-price">
			<%if(item.sale_price>item.deal_price){%><del>￥<%=item.sale_price%></del><br/><%}%>
...
<%//if (su._superUser.Contains(su._currUser.UserName)){
        /*
	if(od.order_state==2 && od.pay_state==1 && su._superUser.Contains(su._currUser.UserName)){
		Response.Write("<a href=\"javascript:;\" price=\""+(od.order_money<od.invoice_money?od.invoice_money:od.order_money)+"\" onclick=\"resetRefund(this,'"+ od.order_no +"')\" style=\"color:#555;font-weight:100;\">退款</a><br/>");
		
	}
	if(backMoney>0)
		Response.Write("<span style=\"color:#999;font-size:12px;font-weight:100;\">已退款 "+ backMoney +"</span><br/>");
         * */
%>
    </td>
    <td class="status">
<%
	...
	if(od.order_state==2 && order.pay_state==2 && od.delivery_state==0){
		//_statusString="<span>等待卖家发货</span><br/>";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td><input type="text" id="oupdateRemark-orderNumber" class="input" value="" disabled="disabled"/></td>
  </tr>
  <tr>
    <td class="left" style="height:36px;" valign="top">备注：</td>
...
function updateRemarkBox(obj, orderNumber) {
    ...
    //$(_oupdateRemarkBoxDialog.dialog).find("#updateRemark-textarea").val($(obj).html());
    return false;
}
</script>
<br/><br/>
